...
gpii.settingsHandlers.invokeSettingsHandler = function (handler, payload) {
    for(var pathBase in payload) {
        for(var i = 0; i < payload[pathBase].length; i++) {
            for(var setting in payload[pathBase][i].settings) {
                var path = [pathBase, setting].join(".");
                if(!payload[pathBase][i].settings[setting].schema) {
                    payload[pathBase][i].settings[setting].schema = {};
                }
                payload[pathBase][i].settings[setting].schema.path = path;
            }
        }
    }
    var worklist = [];
